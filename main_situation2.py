...
from data_prep.xstance_dataset_situation2 import get_datasets_main, get_datasets_target_combine
...
def get_target_feature(datasets_target_list, all_targets_list, model_target, tokenizer):
    ...
    for ti in range(len(all_targets_list)):
        ...
        all_target_feature[ti] = cls_mean  # take average 1113
...
def get_target_relation(weight_matrix):
    ...
    middle = zero_tensor1.scatter_(-1, index, 1)
    target_relation_matrix = torch.zeros([opt.num_target, opt.num_target]).to(opt.device)
    
    # 保证对称性
    for i in range(opt.num_target):
        for j in range(i+1, opt.num_target):
            if middle[i][j] == 1 and middle[j][i] ==1:
                target_relation_matrix[i][j] = 1
                target_relation_matrix[j][i] = 1 
    
    
    # 保证topk取的不是weight中的0值
    ...
    target_relation_matrix += torch.eye(opt.num_target).to(opt.device)
...
def train(opt):
    ...
    all_targets_list = [15, 16, 17, 18, 19, 20, 35, 59, 60, 61, 62, 63, 64, 1449, 1452, 1453, 1493, 1495, 1496, 1497, 2715, 3391, 3427, 3428, 3429, 3430, 3431, 3468, 3469, 3470, 3471]
    src_targets_list = [15, 16, 17, 18, 19, 20, 35, 59, 60, 61, 62, 63, 64, 1449, 1452, 1453, 1493, 1495, 1496, 1497]
    tgt_targets_list = [64, 1449, 1452, 1453, 1493, 1495, 1496, 1497, 2715, 3391, 3427, 3428, 3429, 3430, 3431, 3468, 3469, 3470, 3471]
   
    all_targets_list,  train_dataset, valid_dataset, test_dataset = get_datasets_main(train_file_path, valid_file_path, test_file_path, tokenizer, opt.num_train_lines, opt.max_seq_len, all_targets_list, src_targets_list, tgt_targets_list)
    
    opt.num_target = len(all_targets_list) 
    
    
    datasets_target_list = get_datasets_target_combine(train_file_path, valid_file_path, test_file_path, tokenizer, opt.num_train_lines, opt.max_seq_len, all_targets_list, src_targets_list, tgt_targets_list)
    ...
    norm_similarity_matrix = data_normal(similarity_matrix)
    adj = calculate_adj(norm_similarity_matrix, opt.sim_threshold) # original adj, to be optimized
    adj = adj - torch.eye(opt.num_target).to(opt.device) 
    ...
    for epoch in range(opt.max_epoch):
        ...
        for i, (inputs,y_t, y, y_l, y_d) in tqdm(enumerate(train_iter), total=max_iter_per_epoch):
            ...
            loss_con_target = CTL(features, y, y_l, y_t, target_mask)
            
            # loss_con = opt.beta_l * loss_con_lang + opt.beta_t * loss_con_target
            loss_con = loss_con_target
